...
{
    sln.resize(orig->sln.size());
    for (int i = 0; i < (*data).n_of_events; i++ ){
        pair<int,int> initPair;
        initPair.first = orig->sln[i].first;
        initPair.second = orig->sln[i].second;
        sln[i] = initPair;
    }
    map<int, vector<int> >::iterator it = orig->timeslot_events.begin();
    while(it != orig->timeslot_events.end()) {
        vector<int> copyVector;
        vector<int>::iterator it2 = it->second.begin();
        while (it2 != it->second.end()) {
            copyVector.push_back(*it2);
            ++it2;
        }

        timeslot_events[it->first] = copyVector;
        ++it;
    }
...
int Solution::singleClassesScv(int e)
{
    ...
    for(int i = 0; i < data->n_of_students; i++){
        if(data->student_events[i][e] == 1){
            ...
            // classes = 0 means that the student under consideration has a single class in the day (for event e) but that
            // we are not interested in that here (it is counted in eventScv(e))
            if(classes == 1)
                singleClasses +=1;
        }
    }
...
void Solution::localSearch(int maxSteps, double LS_limit, double prob1, double prob2, double prob3)
{
    ...
    if(!feasible ){ // if the timetable is not feasible try to solve hcv
        for( int i = 0; evCount < data->n_of_events; i = (i+1)% data->n_of_events){
            if(timer.elapsedTime(Timer::REAL) > LS_limit || stepCount > maxSteps )//VIRTUAL
                break;
    ...
    if(feasible){ // if the timetable is feasible
        ...
        for( int i = 0; evCount < data->n_of_events; i = (i+1)% data->n_of_events){ //go through the events in the list
            if(stepCount > maxSteps || timer.elapsedTime(Timer::REAL) > LS_limit)//VIRTUAL
                break;
