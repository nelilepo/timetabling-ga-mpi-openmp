...
Control::Control( int argc, char** argv ) {
    ...
    if( parameterExists( "-c" ) ) {
        threadsNum = getIntParameter( "-c" );
        cout << "Max number of threads " << threadsNum << endl;
    } else {
        cerr << "Warning: Number of threads is set to default (1)" << endl;
        threadsNum = 1; // default number of threads
    }
...
double
Control::getTime() {
    return timer.elapsedTime( Timer::REAL );//VIRTUAL
}
...
Control::endTry( Solution *bestSolution) {
    (*os) << "begin solution " << nrTry << " time: " << timer.elapsedTime(Timer::REAL) << endl;
